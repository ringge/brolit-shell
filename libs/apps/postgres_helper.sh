...
function postgres_list_databases() {
    ...
    if [[ ${stage} == "all" ]]; then
        ...
        # Run command
        databases="$(${PSQL_ROOT} -Bse 'show databases' | grep "${stage}")"
    ...
    if [[ ${postgres_result} -eq 0 && ${databases} != "error" ]]; then
        ...
        log_event "debug" "Last command executed: ${PSQL_ROOT} -Bse 'show databases'" "false"
...
function postgres_list_users() {
    ...
    if [[ ${postgres_result} -eq 0 && ${users} != "error" ]]; then
        ...
        log_event "debug" "Last command executed: ${PSQL_ROOT} -e SELECT user FROM mysql.user;'" "false"
...
function postgres_user_create() {
    ...
    if [[ ${postgres_result} -eq 0 ]]; then
        ...
        log_event "debug" "Last command executed: ${PSQL_ROOT} -e ${query}" "false"
...
function postgres_user_delete() {
    ...
    log_event "info" "Deleting ${db_user} user in Postgres ..."
